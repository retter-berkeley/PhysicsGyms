...
#create gym for Lorenz Attractor
...
#Lorenz equations:
#xdot = sigma(x-y); ydot = rho*x-y-xz; zdot=beta*z-xy
...
class LrzEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, size=30):
        ...
        self.observation_space =spaces.Box(low=-size+1, high=size-1, shape=(3,), dtype=float)
    ...
    def reset(self, seed: Optional[int] = None, options=None):
        ...
        while (z>-0.5 and z<0.5):
            np.random.seed()
            z=np.random.uniform(-8.,8.)
        self.state = np.array([x,y, z])#  self.np_random.uniform(0, 1, size=2, dtype=float)
    ...
    def step(self,action):
        ...
        result=solve_ivp(Lorenz, (0, 0.5), self.state, args=[u])

        x1=result.y[0,-1]
        x2=result.y[1,-1]
        x3=result.y[2,-1]
        self.state=np.array([x1.item(),x2.item(), x3.item()])
        done=False
        observation=self._get_obs()
        info=x1
        done=(math.sqrt(x1.item()**2+x2.item()**2+x3.item()**2)<0.5)# and action<0.5) 
        reward = -math.sqrt(x1.item()**2+x2.item()**2+x3.item()**2)
